...
class Queen {
    ...
    clearColor = async (board) => {
        for (let j = 0; j < n; ++j) {
            ...
            for (let k = 0; k < n; ++k)
                (j + k) & 1
                    ? (row.getElementsByTagName("td")[k].style.backgroundColor = "#4CAF50") // Green color
                    : (row.getElementsByTagName("td")[k].style.backgroundColor = "#2196F3"); // Blue color
        }
    }
    ...
    for (let k = 0; k < array[n]; ++k) {
        ...
        for (let i = 0; i < n; ++i) {
            ...
            for (let j = 0; j < n; ++j) {
                ...
                (i + j) & 1
                    ? (col.style.backgroundColor = "#4CAF50") // Green color
                    : (col.style.backgroundColor = "#2196F3"); // Blue color
                col.innerHTML = "-";
